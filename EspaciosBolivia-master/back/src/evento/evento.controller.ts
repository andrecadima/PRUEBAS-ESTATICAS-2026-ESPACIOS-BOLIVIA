import { BadRequestException, Body, Controller, Delete, Get, InternalServerErrorException, Patch, Post, Put, RequestTimeoutException, Param, ParseIntPipe} from '@nestjs/common';
...
export class EventoController 
{
   ...
   async create(@Body() dto1:EventoDto1)
   {
      try
      {
         ...
         const fechaEventoSoloFecha = dto1.fecha_evento.split('T')[0];
         const dto: EventoDto ={
            nombre : dto1.nombre,
            ...
            fecha_evento: fechaEventoSoloFecha,
   ...
   /*@Get('id-evento')
   async buscarid(@Body() body: {Idevento:number})
   {
      try{
         const {Idevento} = body;
         return await this.eventoservice.findid(Idevento);
      } catch (error) 
      {
         if (error instanceof TimeoutError) 
         {  // Verifica si el error es por tiempo de espera
      ...
      @Get('id-evento/:Idevento')
      async buscarid(@Param('Idevento', ParseIntPipe) Idevento: number) {
        try {
          return await this.eventoservice.findid(Idevento);
        } catch (error) {
          if (error instanceof TimeoutError) {
            // Manejo de error de tiempo de espera
            throw new RequestTimeoutException(
              'La conexión con la base de datos está tardando demasiado. Intenta más tarde.',
            );
          }
